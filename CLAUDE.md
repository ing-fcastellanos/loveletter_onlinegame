...
- **Faltan cuatro de los ocho efectos de carta.** El Hito 1 del PDD está completo (issues #7 a #12) y el Hito 2 tiene el contrato de efectos (issue #13), el Guardia y el Sacerdote (issue #14), y el Barón y la Sirvienta (issue #15): `EFFECTS.Guard`, `EFFECTS.Priest`, `EFFECTS.Baron` y `EFFECTS.Handmaid` en `packages/engine/src/effect.ts` ya son reales, incluida la primera eliminación de todo el motor (`eliminate` en `state.ts`, compartida para que #16–#17 no la dupliquen), los primeros eventos con audiencia restringida producidos de verdad (`GuardGuessed`, `PriestPeeked`, `PlayerEliminated`, `BaronCompared`), y la primera protección real: el avance de turno en `command.ts` limpia `protected` de quien entra al turno, sin importar qué carta se jugó. A diferencia del Guardia, el objetivo del Barón nunca se hace público — solo `PlayerEliminated` puede delatarlo. Príncipe, Rey, Condesa y Princesa siguen apuntando a `noEffect` — descartarlas es un descarte más. Los issues #16 y #17 reemplazan esas cuatro entradas, una o dos a la vez. Tampoco hay validación de contenido de jugada más allá de lo que Guardia, Sacerdote y Barón necesitan para sí mismos (Condesa obligatoria, objetivos legales para el resto de cartas, `legalMoves` — issue #18) ni detección de fin de ronda (mazo vacío al terminar un turno, o un solo jugador en pie — issue #19).
