...
- **No hay efectos de carta reales.** El Hito 1 del PDD está completo (modelo de estado, mazo, preparación de ronda, `PlayerView`, eventos con audiencia, ciclo de turno — issues #7 a #12). El Hito 2 arrancó con el contrato y la tabla de despacho de efectos (issue #13): `packages/engine/src/effect.ts` define `EFFECTS: Record<CardName, EffectHandler>`, y `Command.Discard` ya acepta `target?`/`guess?` genéricos. Pero las ocho entradas de la tabla apuntan al mismo `noEffect` — descartar cualquier carta, incluida la Princesa, sigue sin eliminar a nadie. Los issues #14 a #17 reemplazan esas entradas, una o dos a la vez, con el comportamiento real de cada carta. Tampoco hay validación de contenido de jugada (Condesa obligatoria, objetivos legales, qué parámetro exige cada carta — issue #18, dueño también de `legalMoves`) ni detección de fin de ronda (mazo vacío al terminar un turno, o un solo jugador en pie — issue #19).
