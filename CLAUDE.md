...
- **Faltan dos de los ocho efectos de carta.** El Hito 1 del PDD está completo (issues #7 a #12) y el Hito 2 tiene el contrato de efectos (issue #13), el Guardia y el Sacerdote (issue #14), el Barón y la Sirvienta (issue #15), y el Príncipe y el Rey (issue #16): `EFFECTS.Guard`, `EFFECTS.Priest`, `EFFECTS.Baron`, `EFFECTS.Handmaid`, `EFFECTS.Prince` y `EFFECTS.King` en `packages/engine/src/effect.ts` ya son reales, incluida la primera eliminación de todo el motor (`eliminate` en `state.ts`, compartida para que #17 no la duplique), los primeros eventos con audiencia restringida producidos de verdad (`GuardGuessed`, `PriestPeeked`, `PlayerEliminated`, `BaronCompared`), y la primera protección real: el avance de turno en `command.ts` limpia `protected` de quien entra al turno, sin importar qué carta se jugó. A diferencia del Guardia, el objetivo del Barón nunca se hace público — solo `PlayerEliminated` puede delatarlo. El Príncipe es la primera carta cuyo objetivo puede ser uno mismo (`legalTargetsForPrince`/`resolvePrinceTarget`, separadas de las compartidas): nunca se descarta "sin efecto", y cuando todos los rivales están protegidos el auto-objetivo es obligatorio y explícito. Condesa y Princesa siguen apuntando a `noEffect` — descartarlas es un descarte más. El issue #17 reemplaza esas dos entradas restantes. Tampoco hay validación de contenido de jugada más allá de lo que Guardia, Sacerdote, Barón, Príncipe y Rey necesitan para sí mismos (Condesa obligatoria, `legalMoves` — issue #18) ni detección de fin de ronda (mazo vacío al terminar un turno, o un solo jugador en pie — issue #19).
