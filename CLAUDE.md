...
- **No hay efectos de carta.** El Hito 1 del PDD está completo: modelo de estado ([ADR 0007](docs/decisions/0007-modelo-de-estado-dos-capas-y-turno.md)), mazo y barajado determinista ([ADR 0008](docs/decisions/0008-aleatoriedad-sfc32-semilla-por-ronda.md)), preparación de ronda ([ADR 0009](docs/decisions/0009-preparacion-de-ronda-reparto-y-sorteo.md)), `PlayerView`/`project` (issue #10), eventos con audiencia (issue #11) y el ciclo de turno (issue #12): `applyCommand(state, cmd)` con `Command.Draw`/`Command.Discard`, avanzando entre jugadores y saltando eliminados. Con esto una ronda se juega de punta a punta —sin poderes— hasta vaciar el mazo. Lo que falta es la Fase 2: ningún comando resuelve el efecto de una carta (descartar la Princesa no elimina a nadie todavía), no hay validación de contenido de jugada (Condesa obligatoria, objetivos legales — issue #18, dueño también de `legalMoves`) y no hay detección de fin de ronda (mazo vacío al terminar un turno, o un solo jugador en pie — issue #19).
