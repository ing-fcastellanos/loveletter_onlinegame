...
- **Faltan seis de los ocho efectos de carta.** El Hito 1 del PDD está completo (issues #7 a #12) y el Hito 2 tiene el contrato de efectos (issue #13) más el Guardia y el Sacerdote (issue #14): `EFFECTS.Guard` y `EFFECTS.Priest` en `packages/engine/src/effect.ts` ya son reales, incluida la primera eliminación de todo el motor (`eliminate` en `state.ts`, compartida para que #15–#17 no la dupliquen) y los primeros eventos con audiencia restringida producidos de verdad (`GuardGuessed`, `PriestPeeked`, `PlayerEliminated`). Barón, Sirvienta, Príncipe, Rey, Condesa y Princesa siguen apuntando a `noEffect` — descartarlas es un descarte más. Los issues #15 a #17 reemplazan esas seis entradas, una o dos a la vez. Tampoco hay validación de contenido de jugada más allá de lo que Guardia y Sacerdote necesitan para sí mismos (Condesa obligatoria, objetivos legales para el resto de cartas, `legalMoves` — issue #18) ni detección de fin de ronda (mazo vacío al terminar un turno, o un solo jugador en pie — issue #19).
